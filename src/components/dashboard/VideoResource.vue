...
const source = ref(-1)
const load = async () => {
    const res = await client.get<{ videos: VideoResource[], total: number }>(
        { url: `video_resource/?page_count=${page_size.value}&page_num=${page.value}&source=${source.value}` }
    )
    ...
    for (var i = 0; i < videos.value.length; ++i) {
        const currentPreviewVideoId = videos.value[i].videoId
        videos.value[i].videoPreview = await client.image(currentPreviewVideoId)
    }
}
